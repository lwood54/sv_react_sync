  ...
  values<const T extends Array<keyof typeof state>>(...values: T) {
    ...
    return {
      subscribe() {
        ...
        } as Pick<typeof state, T[number]> & {
          randomizeValue: typeof randomizeValue;
        };
      },
    };
  },
...
function randomizeValue() {
  const svelteFrame: HTMLIFrameElement | null =
    document.querySelector("#svelte-frame");
  console.info("svelteFrame", svelteFrame);

  if (!svelteFrame) {
    return;
  }
  svelteFrame.contentWindow?.postMessage(
    { type: "randomize", value: Math.floor(Math.random() * 100) },
    "*"
  );
}
